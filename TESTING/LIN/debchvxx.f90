...
!>
!>  Eight test ratios will be computed.  The tests will pass if they are  <
!>  THRESH.  There are two cases that are determined by 1 / (SQRT( N ) * EPS).
!>  If that value is  <=  to the component wise reciprocal condition number,
...
!>       A: norm ( abs( Xc - Xt ) / norm ( Xt )  <=  ERRBND( *, nwise_i, bnd_i ) and
...
!>       A: norm ( abs( Xc(j) - Xt(j) ) ) / norm (Xt(j))  <=  ERRBND( *, cwise_i, bnd_i )
!>          for all j .AND. ERRBND( *, cwise_i, bnd_i )  <=  MAX(SQRT(N), 10) * EPS.
!>          If these conditions are met, the test ratio is set to be
...
!            write (*,*) 'nwise2: ', k, nwise_bnd, nwise_err, errthresh
         IF (NCOND  >=  CONDTHRESH) THEN
            ...
            ELSE
               ...
               ELSE IF (NWISE_ERR  /=  0.0D+0) THEN
                  TSTRAT(1) = 1/(16.0*EPS)
               ELSE
                  TSTRAT(1) = 0.0D+0
               END IF
               IF (TSTRAT(1)  >  1.0D+0) THEN
...
!     Condition number tests
         ...
         IF (RCOND  >=  CONDTHRESH .AND. TSTRAT(4)  <  1.0D+0) &
            TSTRAT(4) = 1.0D+0 / TSTRAT(4)

         TSTRAT(5) = NCOND / NWISE_RCOND
         IF (NCOND  >=  CONDTHRESH .AND. TSTRAT(5)  <  1.0D+0) &
            TSTRAT(5) = 1.0D+0 / TSTRAT(5)

         TSTRAT(6) = CCOND / NWISE_RCOND
         IF (CCOND  >=  CONDTHRESH .AND. TSTRAT(6)  <  1.0D+0) &
