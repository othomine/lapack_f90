...
!            write (*,*) 'nwise2: ', k, nwise_bnd, nwise_err, errthresh
         IF (NCOND  >=  CONDTHRESH) THEN
            ...
            ELSE
               ...
               ELSE IF (NWISE_ERR  /=  0.0) THEN
                  TSTRAT(1) = 1/(16.0*EPS)
               ELSE
                  TSTRAT(1) = 0.0
               END IF
               IF (TSTRAT(1)  >  1.0) THEN
                  TSTRAT(1) = 1/(4.0*EPS)
               END IF
            END IF
         ELSE
...
!     Condition number tests
         ...
         IF (RCOND  >=  CONDTHRESH .AND. TSTRAT(4)  <  1.0) &
            TSTRAT(4) = 1.0 / TSTRAT(4)

         TSTRAT(5) = NCOND / NWISE_RCOND
         IF (NCOND  >=  CONDTHRESH .AND. TSTRAT(5)  <  1.0) &
            TSTRAT(5) = 1.0 / TSTRAT(5)

         TSTRAT(6) = CCOND / NWISE_RCOND
         IF (CCOND  >=  CONDTHRESH .AND. TSTRAT(6)  <  1.0) &
