...
!
   IF( IPVTNG == 0 ) THEN
      ISUB = I
      JSUB = J
   ELSE IF( IPVTNG == 1 ) THEN
      ISUB = IWORK( I )
      JSUB = J
   ELSE IF( IPVTNG == 2 ) THEN
      ISUB = I
      JSUB = IWORK( J )
   ELSE IF( IPVTNG == 3 ) THEN
      ISUB = IWORK( I )
      JSUB = IWORK( J )
   END IF
!
...
!
   ...
   IF( IGRADE == 1 ) THEN
      CTEMP = CTEMP*DL( ISUB )
   ELSE IF( IGRADE == 2 ) THEN
      CTEMP = CTEMP*DR( JSUB )
   ELSE IF( IGRADE == 3 ) THEN
      CTEMP = CTEMP*DL( ISUB )*DR( JSUB )
   ELSE IF( IGRADE == 4 .AND. ISUB /= JSUB ) THEN
      CTEMP = CTEMP*DL( ISUB ) / DL( JSUB )
   ELSE IF( IGRADE == 5 ) THEN
      CTEMP = CTEMP*DL( ISUB )*DCONJG( DL( JSUB ) )
   ELSE IF( IGRADE == 6 ) THEN
      CTEMP = CTEMP*DL( ISUB )*DL( JSUB )
   END IF
   ZLATM2 = CTEMP
   RETURN
