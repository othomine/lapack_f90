...
!
         ...
         ELSE
            ...
            call system_clock(count_rate=nb_periods_sec,count=S1_time)
#endif
            CALL DSCAL( M-I, 1.0D+0 / WB, WORK( 2 ), 1 )
#ifdef _TIMER
            call system_clock(count_rate=nb_periods_sec,count=S2_time)
            open(file='results.out', unit=10, position = 'append')
...
!
         ...
         call system_clock(count_rate=nb_periods_sec,count=S1_time)
#endif
         CALL DGEMV( 'Transpose', M-I+1, N-I+1, 1.0D+0, A( I, I ), LDA, &
                     WORK, 1, 0.0D+0, WORK( M+1 ), 1 )
#ifdef _TIMER
         call system_clock(count_rate=nb_periods_sec,count=S2_time)
         open(file='results.out', unit=10, position = 'append')
...
!
         ...
         call system_clock(count_rate=nb_periods_sec,count=S1_time)
#endif
         CALL DGEMV( 'No transpose', M-I+1, N-I+1, 1.0D+0, A( I, I ), &
                     LDA, WORK, 1, 0.0D+0, WORK( N+1 ), 1 )
#ifdef _TIMER
         call system_clock(count_rate=nb_periods_sec,count=S2_time)
         open(file='results.out', unit=10, position = 'append')
         write(10,'(A,F16.10,A)') 'Total time : DGEMV : ',&
...
!
            ...
            ELSE
               ...
               call system_clock(count_rate=nb_periods_sec,count=S1_time)
#endif
               CALL DSCAL( M-KL-I, 1.0D+0 / WB, A( KL+I+1, I ), 1 )
#ifdef _TIMER
               call system_clock(count_rate=nb_periods_sec,count=S2_time)
               open(file='results.out', unit=10, position = 'append')
               write(10,'(A,F16.10,A)') 'Total time : DSCAL : ',&
...
!
            ...
            call system_clock(count_rate=nb_periods_sec,count=S1_time)
#endif
            CALL DGEMV( 'Transpose', M-KL-I+1, N-I, 1.0D+0, &
                        A( KL+I, I+1 ), LDA, A( KL+I, I ), 1, 0.0D+0, &
                        WORK, 1 )
#ifdef _TIMER
            call system_clock(count_rate=nb_periods_sec,count=S2_time)
            open(file='results.out', unit=10, position = 'append')
...
!
            ...
            ELSE
               ...
               call system_clock(count_rate=nb_periods_sec,count=S1_time)
#endif
               CALL DSCAL( N-KU-I, 1.0D+0 / WB, A( I, KU+I+1 ), LDA )
#ifdef _TIMER
               call system_clock(count_rate=nb_periods_sec,count=S2_time)
               open(file='results.out', unit=10, position = 'append')
               write(10,'(A,F16.10,A)') 'Total time : DSCAL : ',&
...
!
            ...
            call system_clock(count_rate=nb_periods_sec,count=S1_time)
#endif
            CALL DGEMV( 'No transpose', M-I, N-KU-I+1, 1.0D+0, &
                        A( I+1, KU+I ), LDA, A( I, KU+I ), LDA, 0.0D+0, &
                        WORK, 1 )
#ifdef _TIMER
            call system_clock(count_rate=nb_periods_sec,count=S2_time)
            open(file='results.out', unit=10, position = 'append')
...
!
      ...
      END IF
!
      IF (I  <=  N) A(KL+I+1:M, I ) = 0.0D+0
!
      IF (I  <=  M) A( I,KU+I+1:N) = 0.0D+0
   ENDDO
   RETURN
!
!     End of DLAGGE
