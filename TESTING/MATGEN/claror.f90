...
!
         ...
         call system_clock(count_rate=nb_periods_sec,count=S1_time)
#endif
         CALL CGEMV( 'C', IXFRM, N, (1.0E+0,0.0E+0), A( KBEG, 1 ), LDA, &
                     X( KBEG ), 1, (0.0E+0,0.0E+0), X( 2*NXFRM+1 ), 1 )
#ifdef _TIMER
         call system_clock(count_rate=nb_periods_sec,count=S2_time)
         open(file='results.out', unit=10, position = 'append')
...
!
         ...
         call system_clock(count_rate=nb_periods_sec,count=S1_time)
#endif
         CALL CGEMV( 'N', M, IXFRM, (1.0E+0,0.0E+0), A( 1, KBEG ), LDA, &
                     X( KBEG ), 1, (0.0E+0,0.0E+0), X( 2*NXFRM+1 ), 1 )
#ifdef _TIMER
         call system_clock(count_rate=nb_periods_sec,count=S2_time)
         open(file='results.out', unit=10, position = 'append')
