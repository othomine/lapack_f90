...
!
      ...
      call system_clock(count_rate=nb_periods_sec,count=S1_time)
#endif
      CALL ZHEMV( 'Lower', N-I+1, TAU, A( I, I ), LDA, WORK, 1, (0.0D+0,0.0D+0), &
                  WORK( N+1 ), 1 )
#ifdef _TIMER
      call system_clock(count_rate=nb_periods_sec,count=S2_time)
      open(file='results.out', unit=10, position = 'append')
...
!
      ALPHA = -(0.5D+0,0.0D+0)*TAU*ZDOTC( N-I+1, WORK( N+1 ), 1, WORK, 1 )
#ifdef _TIMER
      call system_clock(count_rate=nb_periods_sec,count=S1_time)
#endif
...
!
      ...
      call system_clock(count_rate=nb_periods_sec,count=S1_time)
#endif
      CALL ZHER2( 'Lower', N-I+1, -(1.0D+0,0.0D+0), WORK, 1, WORK( N+1 ), 1, &
                  A( I, I ), LDA )
#ifdef _TIMER
      call system_clock(count_rate=nb_periods_sec,count=S2_time)
      open(file='results.out', unit=10, position = 'append')
...
!        compute  v := y - 1/2 * tau * ( y, u ) * u
!
      ALPHA = -(0.5D+0,0.0D+0)*TAU*ZDOTC( N-K-I+1, WORK, 1, A( K+I, I ), 1 )
#ifdef _TIMER
      call system_clock(count_rate=nb_periods_sec,count=S1_time)
#endif
...
!
      ...
      call system_clock(count_rate=nb_periods_sec,count=S1_time)
#endif
      CALL ZHER2( 'Lower', N-K-I+1, -(1.0D+0,0.0D+0), A( K+I, I ), 1, WORK, 1, &
                  A( K+I, K+I ), LDA )
#ifdef _TIMER
      call system_clock(count_rate=nb_periods_sec,count=S2_time)
      open(file='results.out', unit=10, position = 'append')
