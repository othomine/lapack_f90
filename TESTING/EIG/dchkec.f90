...
!
   ...
   IF( RTRSNA( 1 ) > THRESH .OR. RTRSNA( 2 ) > THRESH .OR. &
       NTRSNA( 1 ) /= 0 .OR. NTRSNA( 2 ) /= 0 .OR. NTRSNA( 3 ) /= 0 ) &
        THEN
      OK = .FALSE.
...
!
   ...
   IF( RTRSEN( 1 ) > THRESH .OR. RTRSEN( 2 ) > THRESH .OR. &
       NTRSEN( 1 ) /= 0 .OR. NTRSEN( 2 ) /= 0 .OR. NTRSEN( 3 ) /= 0 ) &
        THEN
      OK = .FALSE.
...
!
   ...
   IF( OK ) WRITE( NOUT, FMT = 9990 )PATH, NTESTS
!
   RETURN
