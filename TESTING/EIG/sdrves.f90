...
!
   ...
   ELSE IF( LDA < 1 .OR. LDA < NMAX ) THEN
      INFO = -9
   ELSE IF( LDVS < 1 .OR. LDVS < NMAX ) THEN
      INFO = -17
...
!
   DO JSIZE = 1, NSIZES
      N = NN( JSIZE )
      MTYPES = MAXTYP
      IF( NSIZES == 1 .AND. NTYPES == MAXTYP+1 ) &
         MTYPES = MTYPES + 1
!
      DO JTYPE = 1, MTYPES
...
!
            ADUMMA( 1 ) = ' '
            CALL SLATME( N, 'S', ISEED, WORK, IMODE, COND, 1.0E+0, &
...
!
            CALL SLATMR( N, N, 'S', ISEED, 'N', WORK, 6, 1.0E+0, 1.0E+0, &
                         'T', 'N', WORK( N+1 ), 1, 1.0E+0, &
                         WORK( 2*N+1 ), 1, 1.0E+0, 'N', IDUMMA, N, N, &
                         0.0E+0, ANORM, 'NO', A, LDA, IWORK, IINFO )
            IF( N >= 4 ) THEN
               ...
               call system_clock(count_rate=nb_periods_sec,count=S1_time)
#endif
               CALL SLASET( 'Full', 2, N, 0.0E+0, 0.0E+0, A, LDA )
#ifdef _TIMER
               call system_clock(count_rate=nb_periods_sec,count=S2_time)
               open(file='results.out', unit=10, position = 'append')
               write(10,'(A,F16.10,A)') 'Total time : SLASET : ',&
               ...
               call system_clock(count_rate=nb_periods_sec,count=S1_time)
#endif
               CALL SLASET( 'Full', N-3, 1, 0.0E+0, 0.0E+0, A( 3, 1 ), &
                            LDA )
#ifdef _TIMER
               call system_clock(count_rate=nb_periods_sec,count=S2_time)
               open(file='results.out', unit=10, position = 'append')
               ...
               call system_clock(count_rate=nb_periods_sec,count=S1_time)
#endif
               CALL SLASET( 'Full', N-3, 2, 0.0E+0, 0.0E+0, A( 3, N-1 ), &
                            LDA )
#ifdef _TIMER
               call system_clock(count_rate=nb_periods_sec,count=S2_time)
               open(file='results.out', unit=10, position = 'append')
               ...
               call system_clock(count_rate=nb_periods_sec,count=S1_time)
#endif
               CALL SLASET( 'Full', 1, N, 0.0E+0, 0.0E+0, A( N, 1 ), &
                            LDA )
#ifdef _TIMER
               call system_clock(count_rate=nb_periods_sec,count=S2_time)
               open(file='results.out', unit=10, position = 'append')
...
!
               ...
               call system_clock(count_rate=nb_periods_sec,count=S2_time)
               open(file='results.out', unit=10, position = 'append')
               write(10,'(A,F16.10,A)') 'Total time : SGEES : ',&
                     real(S2_time-S1_time)/real(nb_periods_sec), ' s'
               close(10)
#endif
...
!
               ...
               DO J = 1, N - 2
                  ...
                  ENDDO
               DO I = 1, N - 2
                  IF( H( I+1, I ) /= 0.0E+0 .AND. H( I+2, I+1 ) /= &
                      0.0E+0 )RESULT( 1+RSUB ) = ULPINV
                  ENDDO
               DO I = 1, N - 1
                  IF( H( I+1, I ) /= 0.0E+0 ) THEN
                     IF( H( I, I ) /= H( I+1, I+1 ) .OR. &
                         H( I, I+1 ) == 0.0E+0 .OR. &
                         SIGN( 1.0E+0, H( I+1, I ) ) == &
                         SIGN( 1.0E+0, H( I, I+1 ) ) )RESULT( 1+RSUB ) &
                         = ULPINV
                  END IF
                  ENDDO
!
               ...
               DO I = 1, N - 1
                  IF( H( I+1, I ) /= 0.0E+0 ) THEN
                     ...
                     RESULT( 4+RSUB ) = MAX( RESULT( 4+RSUB ), &
                                        ...
                                        MAX( ULP*TMP, UNFL ) )
                  ELSE IF( I > 1 ) THEN
                     IF( H( I+1, I ) == 0.0E+0 .AND. H( I, I-1 ) == &
                         0.0E+0 .AND. WI( I ) /= 0.0E+0 )RESULT( 4+RSUB ) &
                          = ULPINV
                  END IF
                  ENDDO
!
