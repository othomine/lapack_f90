...
!
            RESULT( 16 ) = TEMP2 / MAX( UNFL, 100.0D+0*ULP*MAX( TEMP1, TEMP2 ) )
         ELSE
            RESULT( 14:16 ) = 0.0D+0
         END IF
!
...
!              Do test 17
!
            TEMP2 = 2.0D+0*( 2.0D+0*N-1.0D+0 )*ULP*( 1.0D+0+8.0D+0*0.5D+0**2 ) / ( 1.0D+0-0.5D+0 )**4
!
            TEMP1 = 0.0D+0
            DO J = 1, N
...
!
            ...
            IF( JTYPE == 21 .AND. CREL ) THEN
               ...
               call system_clock(count_rate=nb_periods_sec,count=S2_time)
               open(file='results.out', unit=10, position = 'append')
               write(10,'(A,F16.10,A)') 'Total time : ZSTEMR : ',&
                     real(S2_time-S1_time)/real(nb_periods_sec), ' s'
               close(10)
#endif
               IF( IINFO /= 0 ) THEN
