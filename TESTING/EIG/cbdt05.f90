...
!
...
#endif
   CALL CGEMM( 'N', 'C', M, NS, N, (1.0E+0,0.0E+0), A, LDA, VT, &
               LDVT, (0.0E+0,0.0E+0), WORK( 1+NS*NS ), M )
#ifdef _TIMER
   ...
   call system_clock(count_rate=nb_periods_sec,count=S1_time)
#endif
   CALL CGEMM( 'C', 'N', NS, NS, M, -(1.0E+0,0.0E+0), U, LDU, WORK( 1+NS*NS ), &
               M, (0.0E+0,0.0E+0), WORK, NS )
#ifdef _TIMER
   call system_clock(count_rate=nb_periods_sec,count=S2_time)
   open(file='results.out', unit=10, position = 'append')
   write(10,'(A,F16.10,A)') 'Total time : CGEMM : ',&
...
!
   J = 0
   DO I = 1, NS
      WORK(J+I) =  WORK(J+I) + CMPLX(S(I),0.0E+0)
