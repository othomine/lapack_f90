            ...
            DO I = 1, M
               DO J = 1, M
                  X(I+(J-1)*LDX) = X(I+(J-1)*LDX) + &
                                   1.0E-4*SLARND(2,ISEED)
               END DO
            END DO
         ELSE IF( IMAT == 3 ) THEN
            R = MIN( P, M-P, Q, M-Q )
            DO I = 1, R+1
               THETA(I) = 10.0D0**(-SLARND(1,ISEED)*GAPDIGIT)
            END DO
            DO I = 2, R+1
         ...
         ELSE
...
#endif
            CALL SLASET( 'F', M, M, 0.0E+0, 1.0E+0, X, LDX )
#ifdef _TIMER
            ...
            DO I = 1, M
               ...
               IF( J  /=  I ) THEN
                  ...
                  call system_clock(count_rate=nb_periods_sec,count=S2_time)
                  open(file='results.out', unit=10, position = 'append')
                  write(10,'(A,F16.10,A)') 'Total time : SROT : ',&
                        real(S2_time-S1_time)/real(nb_periods_sec), ' s'
                  close(10)
#endif
               END IF
...
!
   ...
   call system_clock(count_rate=nb_periods_sec,count=S1_time)
#endif
   CALL SLASET( 'Full', M, M, 0.0E+0, 0.0E+0, X, LDX )
#ifdef _TIMER
   call system_clock(count_rate=nb_periods_sec,count=S2_time)
   open(file='results.out', unit=10, position = 'append')
   write(10,'(A,F16.10,A)') 'Total time : SLASET : ',&
