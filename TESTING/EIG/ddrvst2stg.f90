...
!
            ...
            IF( N > 0 ) THEN
               IF( IL /= 1 ) THEN
                  VL = WA1( IL ) - MAX( 0.5D0* &
                       ( WA1( IL )-WA1( IL-1 ) ), 10.0D0*ULP*TEMP3, &
                       10.0D0*RTUNFL )
               ELSE
                  VL = WA1( 1 ) - MAX( 0.5D0*( WA1( N )-WA1( 1 ) ), &
                       10.0D0*ULP*TEMP3, 10.0D0*RTUNFL )
               END IF
               IF( IU /= N ) THEN
                  VU = WA1( IU ) + MAX( 0.5D0* &
                       ( WA1( IU+1 )-WA1( IU ) ), 10.0D0*ULP*TEMP3, &
                       10.0D0*RTUNFL )
               ELSE
...
!
            IF( N > 0 ) THEN
               ...
               IF( IL /= 1 ) THEN
                  VL = D1( IL ) - MAX( 0.5D0*( D1( IL )-D1( IL-1 ) ), &
                       10.0D0*ULP*TEMP3, 10.0D0*RTUNFL )
               ELSE IF( N > 0 ) THEN
                  VL = D1( 1 ) - MAX( 0.5D0*( D1( N )-D1( 1 ) ), &
                       10.0D0*ULP*TEMP3, 10.0D0*RTUNFL )
               END IF
               IF( IU /= N ) THEN
                  VU = D1( IU ) + MAX( 0.5D0*( D1( IU+1 )-D1( IU ) ), &
                       10.0D0*ULP*TEMP3, 10.0D0*RTUNFL )
               ELSE IF( N > 0 ) THEN
                  VU = D1( N ) + MAX( 0.5D0*( D1( N )-D1( 1 ) ), &
                       10.0D0*ULP*TEMP3, 10.0D0*RTUNFL )
               END IF
            ELSE
               TEMP3 = 0.0D0
...
!
            ...
            CALL DSYEVX_2STAGE( 'N', 'V', UPLO, N, A, LDU, VL, VU, &
                         ...
                         LWORK, IWORK, IWORK( 5*N+1 ), IINFO )
            IF( IINFO /= 0 ) THEN
               WRITE( NOUNIT, FMT = 9999 ) 'DSYEVX_2STAGE(N,V,' // UPLO // &
                  ')', IINFO, N, JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO < 0 ) THEN
                  RETURN
...
!
            ...
            CALL DSBEV_2STAGE( 'N', UPLO, N, KD, V, LDU, D3, Z, LDU, &
                        WORK, LWORK, IINFO )
            IF( IINFO /= 0 ) THEN
               WRITE( NOUNIT, FMT = 9999 ) 'DSBEV_2STAGE(N,' // UPLO // ')', &
                  IINFO, N, JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO < 0 ) THEN
                  RETURN
...
!
            ...
            CALL DSBEVX_2STAGE( 'N', 'A', UPLO, N, KD, V, LDU, &
                         ...
                         IINFO )
            IF( IINFO /= 0 ) THEN
               WRITE( NOUNIT, FMT = 9999 ) 'DSBEVX_2STAGE(N,A,' // UPLO // &
                  ')', IINFO, N, JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO < 0 ) THEN
                  RETURN
...
!
            ...
            CALL DSBEVX_2STAGE( 'N', 'I', UPLO, N, KD, V, LDU, &
                         ...
                         IINFO )
            IF( IINFO /= 0 ) THEN
               WRITE( NOUNIT, FMT = 9999 ) 'DSBEVX_2STAGE(N,I,' // UPLO // &
                  ')', IINFO, N, JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO < 0 ) THEN
                  RETURN
...
!
            ...
            CALL DSBEVX_2STAGE( 'N', 'V', UPLO, N, KD, V, LDU, &
                         ...
                         IINFO )
            IF( IINFO /= 0 ) THEN
               WRITE( NOUNIT, FMT = 9999 ) 'DSBEVX_2STAGE(N,V,' // UPLO // &
                  ')', IINFO, N, JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO < 0 ) THEN
                  RETURN
...
!
            ...
            IF( IINFO /= 0 ) THEN
               WRITE( NOUNIT, FMT = 9999 ) 'DSYEVD_2STAGE(N,' // UPLO // &
                  ')', IINFO, N, JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO < 0 ) THEN
                  RETURN
...
!
            ...
            IF( IINFO /= 0 ) THEN
               WRITE( NOUNIT, FMT = 9999 ) 'DSBEVD_2STAGE(N,' // UPLO // &
                  ')', IINFO, N, JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO < 0 ) THEN
                  RETURN
