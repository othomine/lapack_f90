...
!
            ...
            DO I = 2, N
               ...
               IF( TEMP1 > 0.5D0 ) THEN
                  A( I-1, I ) = 0.5D0*SQRT( ABS( A( I-1, I-1 )*A( I, I ) ) )
                  A( I, I-1 ) = A( I-1, I )
               END IF
            ENDDO
!
...
!
         ...
         call system_clock(count_rate=nb_periods_sec,count=S1_time)
#endif
         CALL DLASET( 'Full', N, 1, 0.0D0, 0.0D0, SD, N )
#ifdef _TIMER
         call system_clock(count_rate=nb_periods_sec,count=S2_time)
         open(file='results.out', unit=10, position = 'append')
         write(10,'(A,F16.10,A)') 'Total time : DLASET : ',&
         ...
         call system_clock(count_rate=nb_periods_sec,count=S1_time)
#endif
         CALL DLASET( 'Full', N, 1, 0.0D0, 0.0D0, SE, N )
#ifdef _TIMER
         call system_clock(count_rate=nb_periods_sec,count=S2_time)
         open(file='results.out', unit=10, position = 'append')
         write(10,'(A,F16.10,A)') 'Total time : DLASET : ',&
...
!
            RESULT( 16 ) = TEMP2 / MAX( UNFL, 100.0D0*ULP*MAX( TEMP1, TEMP2 ) )
         ELSE
            RESULT( 14:16 ) = 0.0E+0
         END IF
!
...
!              Do test 17
!
            TEMP2 = 2.0D0*( 2.0D0*N-1.0D0 )*ULP*( 1.0D0+8.0D0*0.5D0**2 ) / &
                    ( 1.0D0-0.5D0 )**4
!
            TEMP1 = 0.0D0
            DO J = 1, N
...
!
         IF( N > 0 ) THEN
            IF( IL /= 1 ) THEN
               VL = WA1( IL ) - MAX( 0.5D0*( WA1( IL )-WA1( IL-1 ) ), &
                    ULP*ANORM, 2.0D0*RTUNFL )
            ELSE
               VL = WA1( 1 ) - MAX( 0.5D0*( WA1( N )-WA1( 1 ) ), &
                    ULP*ANORM, 2.0D0*RTUNFL )
            END IF
            IF( IU /= N ) THEN
               VU = WA1( IU ) + MAX( 0.5D0*( WA1( IU+1 )-WA1( IU ) ), &
                    ULP*ANORM, 2.0D0*RTUNFL )
            ELSE
               VU = WA1( N ) + MAX( 0.5D0*( WA1( N )-WA1( 1 ) ), &
                    ULP*ANORM, 2.0D0*RTUNFL )
            END IF
         ELSE
            VL = 0.0D0
...
!                 Do test 28
!
                  TEMP2 = 2.0D0*( 2.0D0*N-1.0D0 )*ULP* &
                          ( 1.0D0+8.0D0*0.5D0**2 ) / ( 1.0D0-0.5D0 )**4
!
                  TEMP1 = 0.0D0
                  DO J = IL, IU
...
!
               IF( N > 0 ) THEN
                  IF( IL /= 1 ) THEN
                     VL = D2( IL ) - MAX( 0.5D0* &
                          ( D2( IL )-D2( IL-1 ) ), ULP*ANORM, 2.0D0*RTUNFL )
                  ELSE
                     VL = D2( 1 ) - MAX( 0.5D0*( D2( N )-D2( 1 ) ), ULP*ANORM, 2.0D0*RTUNFL )
                  END IF
                  IF( IU /= N ) THEN
                     VU = D2( IU ) + MAX( 0.5D0* &
                          ( D2( IU+1 )-D2( IU ) ), ULP*ANORM, 2.0D0*RTUNFL )
                  ELSE
                     VU = D2( N ) + MAX( 0.5D0*( D2( N )-D2( 1 ) ), ULP*ANORM, 2.0D0*RTUNFL )
                  END IF
               ELSE
                  VL = 0.0D0
                  VU = 1.0D0
