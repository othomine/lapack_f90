...
!
            DO JCOL = 1, N
               A( JCOL, JCOL ) = ANORM
               IF( JCOL > 1 ) A( JCOL, JCOL-1 ) = (1.0D0,0.0D0)
            ENDDO
!
...
!
            ...
            END IF
!
            CALL ZLATME( N, 'D', ISEED, WORK, IMODE, COND, (1.0D0,0.0D0), &
...
!
            CALL ZLATMR( N, N, 'D', ISEED, 'N', WORK, 6, 1.0D0, (1.0D0,0.0D0), &
                         'T', 'N', WORK( N+1 ), 1, 1.0D0, &
                         WORK( 2*N+1 ), 1, 1.0D0, 'N', IDUMMA, 0, 0, &
                         0.0D0, ANORM, 'NO', A, LDA, IDUMMA, IINFO )
!
         ELSE IF( ITYPE == 8 ) THEN
!
!              Symmetric, random eigenvalues
!
            CALL ZLATMR( N, N, 'D', ISEED, 'H', WORK, 6, 1.0D0, (1.0D0,0.0D0), &
                         'T', 'N', WORK( N+1 ), 1, 1.0D0, &
...
!
   JTYPE = 0
  160 CONTINUE
   READ (NIUNIT,*, END = 200 )N, NSLCT, ISRT
   IF( N == 0 ) GO TO 200
   JTYPE = JTYPE + 1
   ISEED( 1 ) = JTYPE
   READ (NIUNIT,*) ISLCT(1:NSLCT)
   DO I = 1, N
      READ (NIUNIT,*) A( I,1:N)
      ENDDO
   READ (NIUNIT,*) RCDEIN, RCDVIN
!
