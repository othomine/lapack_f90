...
!
  170       CONTINUE
         RESULT( 13 ) = TEMP1
!
!           For positive definite matrices ( JTYPE > 15 ) call CPTEQR
...
!              Do test 17
!
            TEMP2 = 2.0E+0*( 2.0E+0*N-1.0E+0 )*ULP*( 1.0E+0+8.0E+0*0.5E+0**2 ) / ( 1.0E+0-0.5E+0 )**4
!
            TEMP1 = 0.0E+0
            DO J = 1, N
...
!
            ...
            IF( JTYPE == 21 .AND. CREL ) THEN
               ...
               call system_clock(count_rate=nb_periods_sec,count=S2_time)
               open(file='results.out', unit=10, position = 'append')
               write(10,'(A,F16.10,A)') 'Total time : CSTEMR : ',&
                     real(S2_time-S1_time)/real(nb_periods_sec), ' s'
               close(10)
#endif
               IF( IINFO /= 0 ) THEN
