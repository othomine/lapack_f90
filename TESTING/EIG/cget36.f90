...
!
   ...
   call system_clock(count_rate=nb_periods_sec,count=S1_time)
#endif
   CALL CLASET( 'Full', N, N, (0.0E+0,0.0E+0), (1.0E+0,0.0E+0), Q, LDT )
#ifdef _TIMER
   call system_clock(count_rate=nb_periods_sec,count=S2_time)
   open(file='results.out', unit=10, position = 'append')
   ...
   call system_clock(count_rate=nb_periods_sec,count=S2_time)
   open(file='results.out', unit=10, position = 'append')
   write(10,'(A,F16.10,A)') 'Total time : CTREXC : ',&
         real(S2_time-S1_time)/real(nb_periods_sec), ' s'
   close(10)
#endif
   DO I = 1, N
      DO J = 1, N
         IF( I == J .AND. Q( I, J ) /= (1.0E+0,0.0E+0) ) RES = RES + 1.0E+0 / EPS
         IF( I /= J .AND. Q( I, J ) /= (0.0E+0,0.0E+0) ) RES = RES + 1.0E+0 / EPS
      ENDDO
   ENDDO
!
...
!
   ...
   END IF
   DO I = 1, N
      IF( T2( I, I ) /= DIAG( I ) ) RES = RES + 1.0E+0 / EPS
   ENDDO
!
...
!
   DO J = 1, N - 1
      DO I = J + 1, N
         IF( T2( I, J ) /= (0.0E+0,0.0E+0) ) RES = RES + 1.0E+0 / EPS
         ENDDO
      ENDDO
   IF( RES > RMAX ) THEN
      RMAX = RES
