...
!
   ...
   ELSE IF( LDZ <= 1 .OR. LDZ < NMAX ) THEN
      INFO = -16
   ELSE IF( 2*MAX( NMAX, 3 )**2 > NWORK ) THEN
      INFO = -21
   ELSE IF( 2*MAX( NMAX, 3 )**2 > LIWORK ) THEN
      INFO = -23
   END IF
!
   IF( INFO /= 0 ) THEN
...
!
            KA = MAX( 0, N-1 )
            KB = KA
            CALL DLATMR( N, N, 'S', ISEED, 'H', WORK, 6, 1.0D0, 1.0D0, &
                         'T', 'N', WORK( N+1 ), 1, 1.0D0, &
...
!
               CALL DLATMS( N, N, 'U', ISEED, 'P', WORK, 5, 10.0D0, 1.0D0, &
...
!
               ...
               call system_clock(count_rate=nb_periods_sec,count=S2_time)
               open(file='results.out', unit=10, position = 'append')
               write(10,'(A,F16.10,A)') 'Total time : DSYGV : ',&
                     real(S2_time-S1_time)/real(nb_periods_sec), ' s'
               close(10)
#endif
               IF( IINFO /= 0 ) THEN
...
!
               ...
               call system_clock(count_rate=nb_periods_sec,count=S2_time)
               open(file='results.out', unit=10, position = 'append')
               write(10,'(A,F16.10,A)') 'Total time : DSYGV_2STAGE : ',&
                     real(S2_time-S1_time)/real(nb_periods_sec), ' s'
               close(10)
#endif
               IF( IINFO /= 0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 ) 'DSYGV_2STAGE(V,' // UPLO // &
                     ')', IINFO, N, JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO < 0 ) THEN
                     RETURN
...
!
               ...
               call system_clock(count_rate=nb_periods_sec,count=S2_time)
               open(file='results.out', unit=10, position = 'append')
               write(10,'(A,F16.10,A)') 'Total time : DSYGVD : ',&
                     real(S2_time-S1_time)/real(nb_periods_sec), ' s'
               close(10)
#endif
               IF( IINFO /= 0 ) THEN
...
!
               ...
               call system_clock(count_rate=nb_periods_sec,count=S2_time)
               open(file='results.out', unit=10, position = 'append')
               write(10,'(A,F16.10,A)') 'Total time : DSYGVX : ',&
                     real(S2_time-S1_time)/real(nb_periods_sec), ' s'
               close(10)
#endif
               IF( IINFO /= 0 ) THEN
...
!
               ...
               call system_clock(count_rate=nb_periods_sec,count=S2_time)
               open(file='results.out', unit=10, position = 'append')
               write(10,'(A,F16.10,A)') 'Total time : DSPGV : ',&
                     real(S2_time-S1_time)/real(nb_periods_sec), ' s'
               close(10)
#endif
               IF( IINFO /= 0 ) THEN
...
!
               ...
               call system_clock(count_rate=nb_periods_sec,count=S2_time)
               open(file='results.out', unit=10, position = 'append')
               write(10,'(A,F16.10,A)') 'Total time : DSPGVD : ',&
                     real(S2_time-S1_time)/real(nb_periods_sec), ' s'
               close(10)
#endif
               IF( IINFO /= 0 ) THEN
...
!
               ...
               call system_clock(count_rate=nb_periods_sec,count=S2_time)
               open(file='results.out', unit=10, position = 'append')
               write(10,'(A,F16.10,A)') 'Total time : DSPGVX : ',&
                     real(S2_time-S1_time)/real(nb_periods_sec), ' s'
               close(10)
#endif
               IF( IINFO /= 0 ) THEN
...
!
                  ...
                  call system_clock(count_rate=nb_periods_sec,count=S2_time)
                  open(file='results.out', unit=10, position = 'append')
                  write(10,'(A,F16.10,A)') 'Total time : DSBGV : ',&
                        real(S2_time-S1_time)/real(nb_periods_sec), ' s'
                  close(10)
#endif
                  IF( IINFO /= 0 ) THEN
...
!
                  ...
                  call system_clock(count_rate=nb_periods_sec,count=S2_time)
                  open(file='results.out', unit=10, position = 'append')
                  write(10,'(A,F16.10,A)') 'Total time : DSBGVD : ',&
                        real(S2_time-S1_time)/real(nb_periods_sec), ' s'
                  close(10)
#endif
                  IF( IINFO /= 0 ) THEN
...
!
                  ...
                  call system_clock(count_rate=nb_periods_sec,count=S2_time)
                  open(file='results.out', unit=10, position = 'append')
                  write(10,'(A,F16.10,A)') 'Total time : DSBGVX : ',&
                        real(S2_time-S1_time)/real(nb_periods_sec), ' s'
                  close(10)
#endif
                  IF( IINFO /= 0 ) THEN
