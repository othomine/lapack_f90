...
!
   ...
   call system_clock(count_rate=nb_periods_sec,count=S1_time)
#endif
   CALL DGEMM( 'N', 'N', N, N, N, 1.0D+0, U, LDU, S, LDS, 0.0D+0, &
               WORK( N*N+1 ), N )
#ifdef _TIMER
   call system_clock(count_rate=nb_periods_sec,count=S2_time)
   open(file='results.out', unit=10, position = 'append')
...
!
   ...
   call system_clock(count_rate=nb_periods_sec,count=S1_time)
#endif
   CALL DGEMM( 'N', 'C', N, N, N, -1.0D+0, WORK( N*N+1 ), N, V, LDV, &
               1.0D+0, WORK, N )
#ifdef _TIMER
   call system_clock(count_rate=nb_periods_sec,count=S2_time)
   open(file='results.out', unit=10, position = 'append')
...
!
   ...
   call system_clock(count_rate=nb_periods_sec,count=S1_time)
#endif
   CALL DGEMM( 'N', 'N', N, N, N, 1.0D+0, U, LDU, T, LDT, 0.0D+0, &
               WORK( 2*N*N+1 ), N )
#ifdef _TIMER
   call system_clock(count_rate=nb_periods_sec,count=S2_time)
   open(file='results.out', unit=10, position = 'append')
...
!
   ...
   call system_clock(count_rate=nb_periods_sec,count=S1_time)
#endif
   CALL DGEMM( 'N', 'C', N, N, N, -1.0D+0, WORK( 2*N*N+1 ), N, V, LDV, &
               1.0D+0, WORK( N*N+1 ), N )
#ifdef _TIMER
   call system_clock(count_rate=nb_periods_sec,count=S2_time)
   open(file='results.out', unit=10, position = 'append')
