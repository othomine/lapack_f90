...
!
            ...
            call system_clock(count_rate=nb_periods_sec,count=S2_time)
            open(file='results.out', unit=10, position = 'append')
            write(10,'(A,F16.10,A)') 'Total time : SGEEV : ',&
                  real(S2_time-S1_time)/real(nb_periods_sec), ' s'
            close(10)
#endif
            IF( IINFO /= 0 ) THEN
...
!
            DO J = 1, N
               ...
               ELSE IF( WI( J ) > 0.0E+0 ) THEN
                  TNRM = SLAPY2( SNRM2( N, VR( 1, J ), 1 ), &
                         SNRM2( N, VR( 1, J+1 ), 1 ) )
               END IF
               RESULT( 3 ) = MAX( RESULT( 3 ), &
                             MIN( ULPINV, ABS( TNRM-1.0E+0 ) / ULP ) )
               IF( WI( J ) > 0.0E+0 ) THEN
                  VMX = 0.0E+0
                  VRMX = 0.0E+0
                  DO JJ = 1, N
                     ...
                     IF( VTST > VMX ) &
                        VMX = VTST
                     IF( VR( JJ, J+1 ) == 0.0E+0 .AND. &
                         ABS( VR( JJ, J ) ) > VRMX ) &
                         VRMX = ABS( VR( JJ, J ) )
                     ENDDO
                  IF( VRMX / VMX < 1.0E+0-2.0E+0*ULP ) &
                     RESULT( 3 ) = ULPINV
               END IF
               ENDDO
!
