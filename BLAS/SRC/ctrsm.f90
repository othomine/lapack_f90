           ...
           IF (UPPER) THEN
               DO J = 1,N
                   DO I = 1,M
                       ...
                       ELSE
                           TEMP = TEMP - sum(CONJG(A(1:I - 1,I))*B(1:I - 1,J))
               ...
               DO J = 1,N
                   DO I = M,1,-1
                       ...
                       IF (NOCONJ) THEN
                           ...
                           IF (NOUNIT) TEMP = TEMP/A(I,I)
                       ELSE
                           TEMP = TEMP - sum(CONJG(A(I + 1:M,I))*B(I + 1:M,J))
                           IF (NOUNIT) TEMP = TEMP/CONJG(A(I,I))
                       END IF
                       B(I,J) = TEMP
                   ENDDO
...
!
           IF (UPPER) THEN
               DO J = 1,N
                   ...
                   ENDDO
                   IF (NOUNIT) THEN
                       B(1:M,J) = (1.0E+0,0.0E+0)/A(J,J)*B(1:M,J)
                   END IF
               ENDDO
           ELSE
               DO J = N,1,-1
...
!
           IF (UPPER) THEN
               DO K = N,1,-1
                   IF (NOUNIT) THEN
                       IF (NOCONJ) THEN
                           TEMP = (1.0E+0,0.0E+0)/A(K,K)
                       ELSE
                           TEMP = (1.0E+0,0.0E+0)/CONJG(A(K,K))
                       END IF
                       B(1:M,K) = TEMP*B(1:M,K)
                   END IF
                   DO J = 1,K - 1
                       IF (A(J,K) /= (0.0E+0,0.0E+0)) THEN
                           IF (NOCONJ) THEN
                               TEMP = A(J,K)
                           ELSE
                               TEMP = CONJG(A(J,K))
