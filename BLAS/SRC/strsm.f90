...
!
           IF (UPPER) THEN
               DO J = 1,N
                   IF (ALPHA /= 1.0E+0) B(1:M,J) = ALPHA*B(1:M,J)
                   DO K = M,1,-1
...
!
           ...
           ELSE
               DO J = 1,N
                   DO I = M,1,-1
                       TEMP = ALPHA*B(I,J) - sum(A(I + 1:M,I)*B(I + 1:M,J))
                       IF (NOUNIT) TEMP = TEMP/A(I,I)
                       B(I,J) = TEMP
                   ENDDO
               ENDDO
...
!
           IF (UPPER) THEN
               DO K = N,1,-1
                   IF (NOUNIT) THEN
                       B(1:M,K) = B(1:M,K)/A(K,K)
                   END IF
                   DO J = 1,K - 1
                       B(1:M,J) = B(1:M,J) - A(J,K)*B(1:M,K)
                   ENDDO
                   IF (ALPHA /= 1.0E+0) B(1:M,K) = ALPHA*B(1:M,K)
               ENDDO
           ELSE
               DO K = 1,N
                   IF (NOUNIT) B(1:M,K) = B(1:M,K)/A(K,K)
                   DO J = K + 1,N
                       B(1:M,J) = B(1:M,J) - A(J,K)*B(1:M,K)
                   ENDDO
                   IF (ALPHA /= 1.0E+0) B(1:M,K) = ALPHA*B(1:M,K)
