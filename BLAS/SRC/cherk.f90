...
!
       IF (UPPER) THEN
           IF (BETA == 0.0E+0) THEN
               DO J = 1,N
                   DO I = 1,J - 1
                       C(I,J) = ALPHA*sum(CONJG(A(1:K,I))*A(1:K,J))
                   ENDDO
                   C(J,J) = ALPHA*sum(REAL(CONJG(A(1:K,J))*A(1:K,J)))
               ENDDO
           ELSE
               DO J = 1,N
                   DO I = 1,J - 1
                       C(I,J) = ALPHA*sum(CONJG(A(1:K,I))*A(1:K,J)) + BETA*C(I,J)
                   ENDDO
                   C(J,J) = ALPHA*sum(REAL(CONJG(A(1:K,J))*A(1:K,J))) + BETA*REAL(C(J,J))
              ENDDO
           END IF
       ELSE
           DO J = 1,N
