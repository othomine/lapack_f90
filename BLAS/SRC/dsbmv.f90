...
!
       ...
       IF ((INCX == 1) .AND. (INCY == 1)) THEN
           DO J = 1,N
               ...
               Y(J) = Y(J) + TEMP1*A(KPLUS1,J) + ALPHA*sum(A(K+1-J+MINI:K+1-J+J-1,J)*X(MINI:J-1))
           ENDDO
       ELSE
           JX = KX
           JY = KY
...
!
       IF ((INCX == 1) .AND. (INCY == 1)) THEN
           DO J = 1,N
               TEMP1 = ALPHA*X(J)
               MAXI = MIN(N,J+K)
               Y(J) = Y(J) + TEMP1*A(1,J) + ALPHA*sum(A(1-J+J+1:1-J+MAXI,J)*X(J+1:MAXI))
               Y(J+1:MAXI) = Y(J+1:MAXI) + TEMP1*A(1-J+J+1:1-J+MAXI,J)
           ENDDO
       ELSE
           JX = KX
           JY = KY
