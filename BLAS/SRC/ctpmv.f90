           ...
           IF (INCX == 1) THEN
               DO J = N,1,-1
                   ...
                   IF (NOCONJ) THEN
                       ...
                       TEMP = TEMP + sum(AP(KK-J+1:KK-1)*X(1:J-1))
                   ELSE
                       IF (NOUNIT) TEMP = TEMP*CONJG(AP(KK))
                       TEMP = TEMP + sum(CONJG(AP(KK-J+1:KK-1))*X(1:J-1))
                   END IF
                   X(J) = TEMP
                   KK = KK - J
               ENDDO
       ...
       ELSE
           ...
           IF (INCX == 1) THEN
               DO J = 1,N
                   ...
                   IF (NOCONJ) THEN
                       IF (NOUNIT) TEMP = TEMP*AP(KK)
                       TEMP = TEMP + sum(AP(KK+1:KK-J+N)*X(J+1:N))
                   ELSE
                       IF (NOUNIT) TEMP = TEMP*CONJG(AP(KK))
                       TEMP = TEMP + sum(CONJG(AP(KK+1:KK-J+N))*X(J+1:N))
                   END IF
                   X(J) = TEMP
                   KK = KK + (N-J+1)
               ENDDO
