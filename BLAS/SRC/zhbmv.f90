...
!
   IF ((N == 0) .OR. ((ALPHA == (0.0D+0,0.0D+0)).AND. (BETA == (1.0D+0,0.0D+0)))) RETURN
!
...
!
       ...
       IF ((INCX == 1) .AND. (INCY == 1)) THEN
           DO J = 1,N
               TEMP1 = ALPHA*X(J)
               MINI = MAX(1,J-K)
               Y(MINI:J-1) = Y(MINI:J-1) + TEMP1*A(K+1-J+MINI:K+1-J+J-1,J)
               Y(J) = Y(J) + TEMP1*DBLE(A(KPLUS1,J)) + ALPHA*sum(DCONJG(A(K+1-J+MINI:K,J))*X(MINI:J-1))
           ENDDO
       ELSE
           JX = KX
           JY = KY
...
!
       IF ((INCX == 1) .AND. (INCY == 1)) THEN
           DO J = 1,N
               ...
               MAXI = MIN(N,J+K)
               Y(J) = Y(J) + ALPHA*sum(DCONJG(A(2:1-J+MAXI,J))*X(J+1:MAXI))
