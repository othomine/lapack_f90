...
!
       IF ((INCX == 1) .AND. (INCY == 1)) THEN
           TEMP1 = ALPHA*X(1)
           Y(1) = Y(1) + TEMP1*REAL(AP(KK+1-1))
           KK = KK + 1
           DO J = 2,N
               TEMP1 = ALPHA*X(J)
               TEMP2 = sum(CONJG(AP(KK:KK+J - 2))*X(1:J - 1))
...
!
       IF ((INCX == 1) .AND. (INCY == 1)) THEN
           DO J = 1,N - 1
               TEMP1 = ALPHA*X(J)
               TEMP2 = sum(CONJG(AP(KK + 1:KK + N - J))*X(J + 1:N))
               Y(J + 1:N) = Y(J + 1:N) + TEMP1*AP(KK + 1:KK + N - J)
               Y(J) = Y(J) + TEMP1*REAL(AP(KK)) + ALPHA*TEMP2
               KK = KK + (N-J+1)
           ENDDO
           Y(N) = Y(N) + ALPHA*X(N)*REAL(AP(KK))
       ELSE
           JX = KX
           JY = KY
           DO J = 1,N
