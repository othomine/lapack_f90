...
!
       ...
       IF (INCY == 1) THEN
           DO J = 1,N
               MINI = MAX(1,J-KU)
               MAXI = MIN(M,J+KL)
               Y(MINI:MAXI) = Y(MINI:MAXI) + ALPHA*X(JX)*A(KU+1-J+MINI:KU+1-J+MAXI,J)
               JX = JX + INCX
           ENDDO
       ELSE
           DO J = 1,N
...
!
       ...
       IF (INCX == 1) THEN
           DO J = 1,N
               ...
               MAXI = MIN(M,J+KL)
               IF (NOCONJ) THEN
                   TEMP = sum(A(KU+1-J+MINI:KU+1-J+MAXI,J)*X(MINI:MAXI))
               ELSE
                   TEMP = sum(DCONJG(A(KU+1-J+MINI:KU+1-J+MAXI,J))*X(MINI:MAXI))
               END IF
