           ...
           IF (INCX == 1) THEN
               DO J = 1,N
                   IF (X(J) /= (0.0D+0,0.0D+0)) THEN
                       X(1:J-1) = X(1:J-1) + X(J)*AP(KK:KK+J-2)
           ...
           IF (INCX == 1) THEN
               DO J = N,1,-1
                   IF (NOCONJ) THEN
                       ...
                       X(J) = X(J) + sum(AP(KK-J+1:KK-1)*X(1:J-1))
                   ELSE
                       IF (NOUNIT) X(J) = X(J)*DCONJG(AP(KK))
                       X(J) = X(J) + sum(DCONJG(AP(KK-J+1:KK-1))*X(1:J-1))
                   END IF
                   KK = KK - J
               ENDDO
           ELSE
       ...
       ELSE
           ...
           IF (INCX == 1) THEN
               DO J = 1,N
                   IF (NOCONJ) THEN
                       IF (NOUNIT) X(J) = X(J)*AP(KK)
                       X(J) = X(J) + sum(AP(KK+1:KK-J+N)*X(J+1:N))
                   ELSE
                       IF (NOUNIT) X(J) = X(J)*DCONJG(AP(KK))
                       X(J) = X(J) + sum(DCONJG(AP(KK+1:KK-J+N))*X(J+1:N))
                   END IF
                   KK = KK + (N-J+1)
               ENDDO
           ELSE
