...
!
       ...
       IF ((INCX == 1) .AND. (INCY == 1)) THEN
           Y(1) = Y(1) + ALPHA*X(1)*REAL(A(K + 1,1))
           DO J = 2,N
               MINI = MAX(1,J-K)
               TEMP1 = ALPHA*X(J)
               Y(MINI:J-1) = Y(MINI:J-1) + TEMP1*A(K+1-J+MINI:K,J)
               Y(J) = Y(J) + TEMP1*REAL(A(K + 1,J)) + ALPHA*sum(CONJG(A(K+1-J+MINI:K,J))*X(MINI:J-1))
           ENDDO
       ELSE
           JX = KX
           JY = KY
...
!
       IF ((INCX == 1) .AND. (INCY == 1)) THEN
           DO J = 1,N - 1
               TEMP1 = ALPHA*X(J)
               MAXI = MIN(N,J+K)
               Y(J) = Y(J) + TEMP1*REAL(A(1,J)) + ALPHA*sum(CONJG(A(2:1-J+MAXI,J))*X(J+1:MAXI))
               Y(J+1:MAXI) = Y(J+1:MAXI) + TEMP1*A(2:1-J+MAXI,J)
           ENDDO
           Y(N) = Y(N) + ALPHA*X(N)*REAL(A(1,N))
       ELSE
           JX = KX
           JY = KY
           DO J = 1,N
