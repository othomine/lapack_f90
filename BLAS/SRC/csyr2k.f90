...
!
   IF ((N == 0) .OR. (((ALPHA == (0.0E+0,0.0E+0)).OR. &
       (K == 0)).AND. (BETA == (1.0E+0,0.0E+0)))) RETURN
!
...
!
       IF (UPPER) THEN
           IF (BETA == (0.0E+0,0.0E+0)) THEN
               DO J = 1,N
                   DO I = 1,J
                       C(I,J) = ALPHA*(sum(A(1:K,I)*B(1:K,J)) + sum(B(1:K,I)*A(1:K,J)))
                   ENDDO
               ENDDO
           ELSE IF (BETA /= (1.0E+0,0.0E+0)) THEN
               DO J = 1,N
                   DO I = 1,J
                       C(I,J) = BETA*C(I,J) + ALPHA*(sum(A(1:K,I)*B(1:K,J)) + sum(B(1:K,I)*A(1:K,J)))
                   ENDDO
               ENDDO
           ELSE
               DO J = 1,N
                   DO I = 1,J
                       C(I,J) = C(I,J) + ALPHA*(sum(A(1:K,I)*B(1:K,J)) + sum(B(1:K,I)*A(1:K,J)))
                   ENDDO
               ENDDO
           ENDIF
       ELSE
           IF (BETA == (0.0E+0,0.0E+0)) THEN
               DO J = 1,N
                   DO I = J,N
                       C(I,J) = ALPHA*sum(A(1:K,I)*B(1:K,J)) + ALPHA*sum(B(1:K,I)*A(1:K,J))
                   ENDDO
               ENDDO
           ELSE IF (BETA /= (1.0E+0,0.0E+0)) THEN
               DO J = 1,N
