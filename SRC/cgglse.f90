...
!        Update c1
!
      CALL CGEMV( 'No transpose', N-P, P, -(1.0E+0,0.0E+0), A( 1, N-P+1 ), LDA, &
                  D, 1, (1.0E+0,0.0E+0), C, 1 )
   END IF
!
...
!
   IF( M < N ) THEN
      NR = M + P - N
      IF( NR > 0 ) &
         CALL CGEMV( 'No transpose', NR, N-M, -(1.0E+0,0.0E+0), A( N-P+1, M+1 ), &
                     LDA, D( NR+1 ), 1, (1.0E+0,0.0E+0), C( N-P+1 ), 1 )
   ELSE
      NR = P
   END IF
   IF( NR > 0 ) THEN
      ...
      CALL CAXPY( NR, -(1.0E+0,0.0E+0), D, 1, C( N-P+1 ), 1 )
   END IF
!
