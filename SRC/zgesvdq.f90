...
!>          On exit, if JOBU  /=  'N' or JOBV  /=  'N', the lower triangle of A contains
...
!>          On exit, if, on entry, LCWORK /= -1, CWORK(1:N) contains parameters
...
!
   ...
   ELSE IF ( LDA < MAX( 1, M ) ) THEN
      INFO = -9
   ELSE IF ( LDU < 1 .OR. ( LSVC0 .AND. LDU < M ) .OR. &
          ( WNTUF .AND. LDU < N ) ) THEN
      INFO = -12
   ELSE IF ( LDV < 1 .OR. ( RSVEC .AND. LDV < N ) .OR. &
             ( CONDA .AND. LDV < N ) ) THEN
      INFO = -14
