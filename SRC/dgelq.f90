...
!
   ...
   END IF
   LMINWS = .FALSE.
   IF( ( TSIZE < MAX( 1, MB*M*NBLCKS + 5 ) .OR. LWORK < LWOPT ) &
       .AND. ( LWORK >= LWMIN ) .AND. ( TSIZE >= MINTSZ ) &
       .AND. ( .NOT.LQUERY ) ) THEN
...
!
   ...
   ELSE IF( LDA < MAX( 1, M ) ) THEN
     INFO = -4
   ELSE IF( TSIZE < MAX( 1, MB*M*NBLCKS + 5 ) &
      .AND. ( .NOT.LQUERY ) .AND. ( .NOT.LMINWS ) ) THEN
     INFO = -6
   ELSE IF( ( LWORK < LWREQ ) .AND.( .NOT.LQUERY ) &
      .AND. ( .NOT.LMINWS ) ) THEN
     INFO = -8
   END IF
!
