...
!
         IF( TMAX <= SLAMCH('Overflow') ) THEN
            TSCAL = 1.0E+0 / ( SMLNUM*TMAX )
            DO J = 1, N
               IF( CNORM( J ) <= SLAMCH('Overflow') ) THEN
...
!
                  X(1:J-1) = X(1:J-1)-X( J )*TSCAL*A(1:J-1,J)
                  I = ICAMAX( J-1, X, 1 )
                  XMAX = CABS1( X( I ) )
               END IF
            ELSE
...
!                       x(j+1:n) := x(j+1:n) - x(j) * A(j+1:n,j)
!
                  X(J+1:N) = X(J+1:N)-X( J )*TSCAL*A(J+1:N,J)
...
!
               IF( UPPER ) THEN
                  CSUMJ = CSUMJ + USCAL*SUM(A(1:J-1,J)*X(1:J-1))
               ELSE IF( J < N ) THEN
                  CSUMJ = CSUMJ + USCAL*SUM(A(J+1:N,J)*X(J+1:N))
               END IF
            END IF
!
...
!
               IF( UPPER ) THEN
                  CSUMJ = CSUMJ + USCAL*SUM(CONJG(A(1:J-1,J))*X(1:J-1))
               ELSE IF( J < N ) THEN
                  CSUMJ = CSUMJ + USCAL*SUM(CONJG(A(J+1:N,J))*X(J+1:N))
               END IF
            END IF
!
            IF( USCAL == CMPLX( TSCAL ) ) THEN
