...
!
   IF( LSAME( DIRECT, 'F' ) ) THEN
      ...
      DO I = 1, K
         ...
         IF( TAU( I ) == (0.0E+0,0.0E+0) ) THEN
!
!              H(i)  =  I
!
            T(1:I,I) = (0.0E+0,0.0E+0)
         ELSE
!
!              general case
!
...
!                 Skip any trailing zeros.
               DO LASTV = N, I+1, -1
                  IF( V( LASTV, I ) /= (0.0E+0,0.0E+0) ) EXIT
               END DO
               T(1:I-1,I) = -TAU(I) * CONJG(V(I,1:I-1))
...
!                 Skip any trailing zeros.
               DO LASTV = N, I+1, -1
                  IF( V( I, LASTV ) /= (0.0E+0,0.0E+0) ) EXIT
               END DO
...
!                    Skip any leading zeros.
                  DO LASTV = 1, I-1
                     IF( V( LASTV, I ) /= (0.0E+0,0.0E+0) ) EXIT
                  END DO
                  T(I+1:K,I) = -TAU(I) * CONJG(V(N-K+I,I+1:K))
