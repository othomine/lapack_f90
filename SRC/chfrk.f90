...
!
!     The quick return case: ((ALPHA == 0).AND.(BETA /= 0.0E+0)) is not
!     done (it is in CHERK for example) and left in the general case.
!
   IF( ( N == 0 ) .OR. ( ( ( ALPHA == 0.0E+0 ) .OR. ( K == 0 ) ) .AND. ( BETA == 1.0E+0 ) ) )RETURN
!
   IF( ( ALPHA == 0.0E+0 ) .AND. ( BETA == 0.0E+0 ) ) THEN
      C(1:(N*(N+1))/2) = (0.0E+0,0.0E+0)
      RETURN
   END IF
!
   CALPHA = CMPLX( ALPHA, 0.0E+0 )
   CBETA = CMPLX( BETA, 0.0E+0 )
!
!     C is N-by-N.
