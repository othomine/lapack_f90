...
!
         CALL CGERU( K-1, NRHS, -(1.0E+0,0.0E+0), AP( KC ), 1, B( K, 1 ), LDB, &
                     B( 1, 1 ), LDB )
!
!           Multiply by the inverse of the diagonal block.
!
         S = REAL( (1.0E+0,0.0E+0) ) / REAL( AP( KC+K-1 ) )
         B(K,1:NRHS) = S*B(K,1:NRHS)
         K = K - 1
      ELSE
!
...
!
         CALL CGERU( K-2, NRHS, -(1.0E+0,0.0E+0), AP( KC ), 1, B( K, 1 ), LDB, &
                     B( 1, 1 ), LDB )
         CALL CGERU( K-2, NRHS, -(1.0E+0,0.0E+0), AP( KC-( K-1 ) ), 1, &
...
!
         ...
         B_TMP(1:NRHS) = B(K-1,1:NRHS)/AKM1K
         B(K-1,1:NRHS) = (AK*B_TMP(1:NRHS)-B(K,1:NRHS)/CONJG(AKM1K))/DENOM
         B(K,1:NRHS) = (AKM1*B(K,1:NRHS)/CONJG(AKM1K)-B_TMP(1:NRHS))/DENOM
         KC = KC - K + 1
         K = K - 2
      END IF
!
...
!
         IF( K < N ) &
            CALL CGERU( N-K, NRHS, -(1.0E+0,0.0E+0), AP( KC+1 ), 1, B( K, 1 ), &
                        LDB, B( K+1, 1 ), LDB )
!
!           Multiply by the inverse of the diagonal block.
!
         S = REAL( (1.0E+0,0.0E+0) ) / REAL( AP( KC ) )
         B(K,1:NRHS) = S*B(K,1:NRHS)
         KC = KC + N - K + 1
         K = K + 1
      ELSE
...
!
         IF( K < N-1 ) THEN
            CALL CGERU( N-K-1, NRHS, -(1.0E+0,0.0E+0), AP( KC+2 ), 1, B( K, 1 ), &
                        LDB, B( K+2, 1 ), LDB )
            CALL CGERU( N-K-1, NRHS, -(1.0E+0,0.0E+0), AP( KC+N-K+2 ), 1, &
...
!
         ...
         DENOM = AKM1*AK - (1.0E+0,0.0E+0)
         B_TMP(1:NRHS) = B(K,1:NRHS)/CONJG(AKM1K)
         B(K,1:NRHS) = ( AK*B_TMP(1:NRHS)-B(K+1,1:NRHS)/AKM1K ) / DENOM
         B(K+1,1:NRHS) = ( AKM1*B(K+1,1:NRHS)/AKM1K-B_TMP(1:NRHS) ) / DENOM
         KC = KC + 2*( N-K ) + 1
         K = K + 2
      END IF
!
