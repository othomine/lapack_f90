...
!
   ...
   ELSE IF( LDC < MAX( 1, M ) ) THEN
      INFO = -11
   ELSE IF( .NOT.LQUERY .AND. LIWORK < IWORK(1) ) THEN
      INFO = -14
   ELSE IF( .NOT.LQUERY .AND. LDSWORK < MAX( NBA, NBB ) ) THEN
      INFO = -16
   END IF
   IF( INFO /= 0 ) THEN
...
!
   IF( MIN( NBA, NBB ) == 1 .OR. LDSWORK < MAX( NBA, NBB ) .OR. &
       LIWORK < IWORK(1) ) THEN
...
!               A( M, M ) is a 1-by-1 block
             CYCLE
          END IF
          IF( A( L, L+1 ) /= ZERO .AND. A( L+1, L ) /= ZERO ) THEN
!               Check if 2-by-2 block is split
             IF( L + 1  ==  IWORK( K + 1 ) ) THEN
...
!               B( N, N ) is a 1-by-1 block
             CYCLE
          END IF
          IF( B( L, L+1 ) /= ZERO .AND. B( L+1, L ) /= ZERO ) THEN
!               Check if 2-by-2 block is split
             IF( L + 1  ==  IWORK( PC + K + 1 ) ) THEN
    ...
    END DO
    IWORK( PC + NBB + 1 ) = N + 1
    IF( IWORK( PC + NBB ) >= IWORK( PC + NBB + 1 ) ) THEN
